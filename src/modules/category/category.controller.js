...
export const createCategories = async (req, res, next) => {
  ...
  const category = await categoryModel.create({
    name, slug: createArabicSlug(name),
...
export const updateCategories = async (req, res, next) => {
  ...
  if (await categoryModel.findOne({ name: req.body.name, _id: { $ne: categoryId } })) {
  ...
  if (subcategories.length > 0) {
    if (req.body.status === 'Inactive') {
      await subcategoryModel.updateMany({ categoryId: categoryId }, { status: 'Inactive' });
     
      await productModel.updateMany({ subcategoryId: { $in: subcategories.map(sub => sub._id) } }, { status: 'Inactive' });
    } else if (req.body.status === 'Active') {
      await subcategoryModel.updateMany({ categoryId: categoryId }, { status: 'Active' });
     
      await productModel.updateMany({ subcategoryId: { $in: subcategories.map(sub => sub._id) } }, { status: 'Active' });
    }
  }
