...
export const createOrder = async (req, res, next) => {
  ...
  for (let product of req.body.products) {
    ...
    let variant = checkProduct.variants.find(
      (variant) =>
        variant.itemNo === product.itemNo && variant.inStoke >= product.quantity
    );
    if (!variant) {
      return next(
        new AppError(
  ...
  if (order) {
    ...
    await cartModel.updateOne({ userId: req.user._id },{$set: { products: [], total: 0 }} );
...
export const getAllOrders = async (req, res) => {
  ...
  const ordersWithDetails = orders.map((order) => {
    const products = order.products.map((product) => {
      ...
      const purchasedVariant = product.productId.variants.find(variant => 
        variant.finalPrice === product.unitPrice);
      ...
      return {
        ...
        itemNo: purchasedVariant ? purchasedVariant.itemNo : 'No matching variant' // إضافة itemNo
      };
    });
...
export const getPendingOrders = async (req, res,next) => {
  const orders = await orderModel.find({ status: "pending" }).populate({
    path: "products.productId",
    select: "name variants",
    populate: {
      path: 'variants',
...
export const getConfirmedOrders = async (req, res,next) => {
  const orders = await orderModel.find({ status: "confirmed" }).populate({
    path: "products.productId",
    select: "name variants",
    populate: {
      path: 'variants',
...
export const getDeliveredOrders = async (req, res,next) => {
  const orders = await orderModel.find({ status: "delivered" }).populate({
    path: "products.productId",
    select: "name variants",
    populate: {
      path: 'variants',
  ...
  const ordersWithProductDetails = orders.map(order => {
    const products = order.products.map(product => {
      ...
      return {
        productId: product.productId._id,
        productName: product.productId.name,
        itemNo: purchasedVariant ? purchasedVariant.itemNo : 'No matching variant',
        quantity: product.quantity,
        unitPrice: product.unitPrice,
        finalPrice: product.finalPrice,
      };
...
const count =ordersWithProductDetails.length
  res.status(200).json({ message: "success",count, orders: ordersWithProductDetails });
};
//     Change Status
