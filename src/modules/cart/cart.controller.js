...
export const updateQuantity = async (req, res) => {
  ...
  cart.products = cart.products.map(product => {
    if (product.productId.toString() === req.params.productId && product.itemNo === itemNo) {
      product.quantity += inc;
      totalUpdated += inc;

      const unitPrice = product.price; 
      const discount = product.discount || 0; 
      product.finalPrice = parseFloat((unitPrice - (unitPrice * discount / 100)).toFixed(2)) * product.quantity;
    }
    return product;
  });
  ...
  cart.total = cart.products.reduce((acc, product) => acc + product.finalPrice, 0);
