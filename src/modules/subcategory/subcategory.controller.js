...
export const createSubCategory = async (req, res,next) => {
    ...
    const subCategory = await subcategoryModel.create({
        name, slug: createArabicSlug(name), categoryId,
...
export const getActivesubCategory = async (req, res, next) => {
    ...
    const subcategory = await subcategoryModel.find({ categoryId: id, status: 'Active' }).select('name image slug').populate({
        path: 'categoryId'
    });;
...
export const updatesubCategories = async (req, res, next) => {
    ...
    if (await subcategoryModel.findOne({ name: req.body.name, _id: { $ne: subcategoryId } })) {
