...
function generateCustomerInformation(doc, invoice) {
  ...
  doc
    ...
    .text(formatCurrency(invoice.subtotal * 100), 150, customerInformationTop + 30)
    .font('Amiri')
    .text(reverseWords(invoice.shipping.name), 300, customerInformationTop)

    .text(reverseWords(invoice.shipping.address), 300, customerInformationTop + 15)
    .text(reverseWords(invoice.shipping.phone), 300, customerInformationTop + 30)
    .moveDown();
...
function generateInvoiceTable(doc, invoice) {
  ...
  generateTableRow(doc, invoiceTableTop, "Name", "Quantity", "Unit Price", "Total");
  generateHr(doc, invoiceTableTop + 20);

  doc.font('Amiri');
  invoice.items.forEach((item, index) => {
    const position = invoiceTableTop + (index + 1) * 30;
    generateTableRow(doc, position, reverseWords(item.name), item.quantity.toString(), formatCurrency(item.unitPrice * 100), formatCurrency(item.finalPrice * 100));
    generateHr(doc, position + 20);
  });

  const subtotalPosition = invoiceTableTop + (invoice.items.length + 1) * 30;
  doc.font('Helvetica');
  generateTableRow(doc, subtotalPosition, "", "", "Subtotal", formatCurrency(invoice.subtotal * 100));
}
